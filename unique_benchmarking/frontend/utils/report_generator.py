...
class EnhancedReportGenerator:
    ...
    def _process_experiment_data(
        ...
        for question_data in questions_map.values():
            ...
            question_data['success_rate'] = (successful / total * 100) if total > 0 else 0
        ...
        return {
            ...
            'results': self._format_legacy_results(responses_data)  # For backward compatibility
        }
        ...
        for ref in references:
            if isinstance(ref, dict):
                ...
                for key, value in ref.items():
                    if isinstance(value, str) and key in ['title', 'description', 'content', 'snippet']:
                ...
                processed_refs.append({
                    'text': self._convert_markdown_to_html(ref),
                    'raw_text': ref
                })
            else:
    ...
    def _calculate_average_times(self, responses_data: List[Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
        ...
        return averages if any(
            any(times.values()) for times in averages.values()
        ) else {}
    
    def _format_legacy_results(self, responses_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _calculate_response_time(self, start_time: str | None, end_time: str | None) -> float:
    ...
    def save_report(self, html_content: str, filename: str, output_dir: str | None = None) -> str:
...
def generate_experiment_report(
    ...
    return generator.generate_enhanced_report(experiment_data, responses_data, golden_answers_data)
...
def generate_and_save_experiment_report(
    ...
    generator = EnhancedReportGenerator()
    return generator.generate_and_save_report(experiment_data, responses_data, golden_answers_data, output_dir)
